...
plot3data <- subset(datatable, subset=(Date >= "2007-02-01" & Date <= "2007-02-02"))
datetime <- paste(as.Date(plot3data$Date), plot3data$Time)
plot3data$DateTime <- as.POSIXct(datetime)
with(plot3data, { plot(Sub_metering_1~DateTime, type="l", ylab="Global Active Power (kilowatts)", xlab="")lines(Sub_metering_2~DateTime,col='Red')lines(Sub_metering_3~DateTime,col='Blue')})
