...
plt2data <- dataTable[dataTable$Date >= "2007-02-01" & Date <= "2007-02-02",]
rm(dataTable)
datetime <- paste(as.Date(plot2data$Date), plot2data$Time)
datetime2 <- as.POSIXct(datetime)
plot(plot2data$Global_active_power~datetime2, type="l",ylab="Global Active Power (kilowatts)", xlab="")
